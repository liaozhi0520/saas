...
class updateIssueDetailForm(forms.Form):
    TYPE_CHOICES = (
        ('1', 'assignment'),
        ('2', 'bug'),
        ('3', 'feature discussing')
    ...
    def __init__(self, request, *args, **kwargs):
        super(updateIssueDetailForm, self).__init__(args,kwargs)
        issue_creator_or_manager_or_members_choices = [('', '---select someone you like---')]
        users_list = ProjectUser.objects.filter(project=request.tracer.project).values_list()
        issue_creator_or_manager_or_members_choices.extend(users_list)
        self.request = request
        current_issue_manager_id=request.tracer.issue.manager.id
        current_issue_members_ids=IssueMember.objects.filter(issue=request.tracer.issue,relation='3')
        self.fields['issue_manager'] = forms.ChoiceField(
            choices=issue_creator_or_manager_or_members_choices,
            label='Manager',
            widget=widgets.Select(attrs={
